...
namespace App\Monkeypatch\CosmosDb;

use Jupitern\CosmosDb\QueryBuilder;
use Jupitern\CosmosDb\CosmosDbCollection;

class QueryBuilderCustom
{
    private $collection = "";
    ...
    public function findAll($isCrossPartition = false)
    {
        ...
        $query = "SELECT {$fields} FROM {$this->from} {$this->join} {$where} {$order} OFFSET {$offset} LIMIT {$limit}";
